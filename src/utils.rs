...
impl GrammarValidator for SqlNullValidator {
    fn validate(&self, sql: &str) -> String {
        ...
        let sql = sql.replace(" = NULL", " IS NULL")
...
impl SqlKeywordValidator {
    ...
    pub fn new(format: SqlCaseFormat) -> Self {
        ...
        let keywords = vec![
            ...
            "IS NOT NULL", "AS", "DISTINCT", "UNION", "ALL", "INSERT", 
            "UPDATE", "DELETE", "CREATE", "ALTER", "DROP", "TABLE", "VIEW",
            "INDEX", "CONSTRAINT", "PRIMARY KEY", "FOREIGN KEY", "REFERENCES"
...
impl ValidatorRegistry {
    ...
    pub fn register<V: GrammarValidator + 'static>(&mut self, name: &str, validator: V) -> &mut Self {
        self.validators.push((name.to_string(), Arc::new(Box::new(validator))));
        self
    }
    ...
    pub fn list_validators(&self) -> Vec<String> {
        self.validators.iter().map(|(name, _)| name.clone()).collect()
    }
    ...
    pub fn register_defaults(&mut self) -> &mut Self {
        self.register("noop", NoopValidator)
            ...
            .register("sql_uppercase", SqlKeywordValidator::new(SqlCaseFormat::Uppercase))
            .register("sql_lowercase", SqlKeywordValidator::new(SqlCaseFormat::Lowercase))
            .register("sql_capitalize", SqlKeywordValidator::new(SqlCaseFormat::Capitalize))
    }
}
...
mod tests {
    ...
    fn test_validator_registry() {
        let mut registry = ValidatorRegistry::new();
        registry.register("test_uppercase", SqlKeywordValidator::new(SqlCaseFormat::Uppercase));

...
use grammar_gen::utils::{
    SqlCaseFormat, SqlNullValidator, SqlKeywordValidator, 
    ValidatorExt, ValidatorRegistry
};
use std::error::Error;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !Path::new(grammar_path).exists() {
        return Err("SQL grammar file not found. Please ensure examples/sql_grammar.txt exists.".into());
    }

    // 1. Demonstrate SQL NULL value handling
    println!("=== SQL NULL Value Handling ===");
    let grammar = Grammar::from_file(grammar_path, "query")?
        .with_validator(Box::new(SqlNullValidator));
    ...
    let grammar = Grammar::from_file(grammar_path, "query")?
        .with_validator(Box::new(chained_validator));
...
fn test_case_formatting(grammar_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    let capitalize_grammar = Grammar::from_file(grammar_path, "query")?
        .with_validator(Box::new(SqlKeywordValidator::new(SqlCaseFormat::Capitalize)));
